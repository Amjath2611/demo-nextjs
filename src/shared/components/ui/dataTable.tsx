...
import { Pet, PetApi } from '@/api/api';
...
const DataTable = ({ data, isFilter }: DataTableProps) => {
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      const petApi = new PetApi(); // Ensure that petApi is properly initialized
      const res = await petApi.updatePet(editedItem);
      if (res) {
  ...
  const handleDeleteSubmit = async (deletedItem:any) => {
    ...
    try {
      const res = await petApi.deletePet(deletedItem?.id, 'special-key');
      if (res) {
        router.refresh();
      }
    } catch (error) {
